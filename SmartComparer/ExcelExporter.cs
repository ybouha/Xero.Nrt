...
namespace SmartComparer
{
    public class ExcelExporter<T>
    {
        ...
        private int AddListWorksheets(ExcelPackage package, string worksheetName, List<T>? dataList, int index)
        {
            ...
            // Pre-compile property getters once — avoids reflection overhead per cell
            var getters = typeMembers.Select(p =>
            {
            ...
            while (remainingRows > 0)
            {
                ...
                for (int i = 0; i < typeMembers.Length; i++)
                    worksheet.Cells[1, i + 1].Value = typeMembers[i].Name;
                ...
                for (; row < remainingRows && rowsAdded < MaxRowsPerSheet; row++, rowsAdded++)
                {
                    ...
                    for (int col = 0; col < getters.Length; col++)
                        worksheet.Cells[row + 2, col + 1].Value = getters[col](rowData);
                }
